...
async def main_test():
    ...
    await User.objects.insert_async(new_users)  # 异步操作
    ...
    # 查询一条数据
    # user_obj = User.objects.get(user_name='kingname')
    user_obj = await User.objects.get_async(user_name='new_name')  # 异步操作
    logging.info(f'取到用户： {user_obj.user_name}')

    user_obj3 = User.objects.filter(id='6679435754deefcb7b0fe650').first()
    # user_obj3 = User.objects(pk='6679435754deefcb7b0fe650').first()  # 效果同上。 直接将条件写到 objects 也是可以的， id 字段也可以写成 pk
    user_obj4 = await User.objects.filter(id='6679435754deefcb7b0fe650').first_async()  # 异步操作
    logging.info(f'再次取到用户： {user_obj4.user_name}')
    ...
    a = [o async for o in User.objects.in_async(['67e12678d2220794aebd3e6c', '67e12678d2220794aebd3e6d'])]
    logging.info(f'in 用户数： {len(a)}')
    c = await User.objects.filter(user_name='User B').count_async()
    logging.info(f'统计用户数： {c}')
    d = await User.objects.filter(user_name='User B').delete_async()
    logging.info(f'批量删除用户： {d}')
    f = await User.objects.filter(user_name='User C').update_async(email='xxx@yyy.com')
    logging.info(f'批量更新用户： {f}')


    # 查询多条数据
    users = User.objects.filter(user_type=UserEnum.USER).all()
    users = await User.objects.filter(user_type=UserEnum.USER).all_async()  # 异步操作
    ...
    first_user = await User.objects.order_by("+created_at").first_async()  # 异步操作
    logging.info(f'order_by 取到用户： {first_user.user_name}')
    # 多个排序条件
    second_user = User.objects.order_by("+user_type", "-created_at").first()
    second_user = await User.objects.order_by("+user_type", "-created_at").first_async()  # 异步操作
    ...
    logging.info(f'count无参数统计到 {await users2.count_async()} 个用户')  # 异步操作
    ...
    users = await app.models['User'].objects.filter(others__projects__ne=[]).all_async()  # 异步操作
    ...
    or_user = await queryset.first_async()  # 异步操作
    ...
    manage_count = await User.objects.filter(user_type=UserEnum.MANAGER).count_async()  # 异步操作
    ...
    delete_user.delete()
    await delete_user.delete_async()  # 异步操作
    logging.info(f'删除一个用户后，剩 {User.objects.count()} 个用户')

    # 批量删除
    User.objects.filter(user_type=UserEnum.MANAGER).delete()
    await User.objects.filter(user_type=UserEnum.MANAGER).delete_async()  # 异步操作
