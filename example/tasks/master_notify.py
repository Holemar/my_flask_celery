...
class NotifyTask(Task, ABC):
    name = "fc_agent.tasks.master_notify"
    max_retries = 3
...
@current_app.task(base=NotifyTask, name='fc_agent.tasks.master_notify', queue=settings.NOTIFY_TASK_QUEUE, bind=True)  # , priority=0)
def process(self):
    _id = uuid.uuid4()
    _t = int(time.time())
    retries = int(self.request.retries)  # 重试次数
